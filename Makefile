...
################################################
ALLLINUXLIB =`pkg-config --libs allegro-5` `pkg-config --libs allegro_acodec-5` `pkg-config --libs allegro_audio-5` `pkg-config 
--libs allegro_color-5` `pkg-config --libs allegro_dialog-5` `pkg-config --libs allegro_font-5` `pkg-config --libs allegro_image-5` 
`pkg-config --libs allegro_main-5` `pkg-config --libs allegro_memfile-5` `pkg-config --libs allegro_physfs-5` `pkg-config 
--libs allegro_primitives-5` `pkg-config --libs allegro_ttf-5` `pkg-config --libs allegro_video-5`
ALLWINLIB = -l allegro -l allegro_audio -l allegro_acodec -l allegro_color -l allegro_font -l allegro_image -l allegro_primitives 
-l allegro_ttf
RPILINUXLIB = ../libs/joydisp/disdrv.h ../libs/joydisp/joydrv.h ../libs/audio/SDL1/libaudio.h
RPILIBS = -I/usr/local/include -L/usr/local/lib -lSDL -lpthread
################################################
...
OBJS2 = Backend/main.o Backend/ingame_stats.o Backend/scoretable.o Backend/FSM_routines.o Frontend/Raspi/menu_front.o Frontend/Raspi/game_front.o ${EVENTQ_OBJECT} ${TIMER_OBJECT} 
../libs/joydisp/disdrv.o ../libs/joydisp/joydrv.o ../libs/audio/SDL1/libAudioSDL1.o
################################################
